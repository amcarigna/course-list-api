...
1. Before using it for the first time, be sure to install the required python packages to your virtual environment by running `pip install -r requirements.txt`. Also, create `secret_key.py` in the parent directory. Inside, create a variable called `SECRET_KEY` and assign a random string to it.
